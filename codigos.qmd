...
# <i class="fa-brands fa-r-project"></i> Começando a programar em R {#exemplos-de-códigos}
...
```{r}
#| include: false
# Banco com nomes pouco apropriados (exemplo didático)
dados_ruins_dic <- data.frame(
  `Tratamento (Adubação)` = rep(c("T1 - Testemunha", 
                                  "T2 - 50kg N", 
                                  "T3 - 100kg N", 
                                  "T4 - 150kg N"), each = 5),
  ...
  "Altura da planta (cm)" = c(
    ...
    220, 225, 218, 222, 221    # T4 (maior adubação)
  ),
  "Matéria seca (g)" = c(
...
colnames(dados_ruins_dic) <- c("Repetição", "Tratamento", "Altura da planta (cm)", "Matéria seca (g)")
```
...
3.  **Automático usando o pacote `janitor`**\
...
1.  Pode-se convertê-las em fatores usando a função `as.factor()`:
...
2.  Pode-se convertê-las em fatores usando a função `factor()`:
...
dados_agro$Tratamento <- factor(
  ...
  levels = c("T1", "T2", "T3"),                 # ordem dos níveis
  labels = c("Controle", "Adubo", "Bioestimulante") # nomes legíveis
)
...
3.  Pode-se ainda convertê-las em fatores usando a função `convert_as_factor()` do pacote {rstatix}:
...
### Aula 8 – Estatística Descritiva e Gráficos
